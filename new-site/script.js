...
function initTheme() {
    ...
    const isDark = savedTheme === 'dark' || (!savedTheme && shouldBeDark());
...
function updateThemeIcons(isDark) {
    ...
    toggles.forEach(toggle => {
        if (toggle) {
            const iconSpan = toggle.querySelector('.material-icons');
...
function fetchDashboardData() {
    if (!scheduleContainer || !announcementsContainer || !homeworkContainer) return;
    try {
